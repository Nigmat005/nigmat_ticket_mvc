...
public abstract class AbstractMapMockDB<Id,T>{
    ...
    public List<T> findAll(){
      ...
      if(list.stream(). anyMatch(each->each instanceof UserDTO) ){
          list=list.stream().filter(userDTO-> ((UserDTO) userDTO).isEnabled())
                  .collect(Collectors.toList());
      } else if (list.stream().anyMatch(each->each instanceof ProjectDTO)) {
          list=list.stream().filter(eachProjectDTO-> !((ProjectDTO) eachProjectDTO).isDeleted())
                  .collect(Collectors.toList());
      }
        return list;
    }
    ...
    public void softDeleteById(Id id){
        ...
        if(object instanceof UserDTO){
            ...
            ((ProjectDTO) object).setDeleted(true);
        }
